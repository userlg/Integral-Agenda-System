<template>
	<div class="session-wrapper"  style="background-color: white;">
		...
		<div class="session-right text-xs-center">
			<div class="session-table-cell">
				<div class="session-content">
					...
					<p class="fs-14">{{$t('message.loginmessage')}}</p>
					<v-form v-model="valid" class="mb-4">
						...
						<div>
							<v-btn round @click="submit" block color="primary">{{$t('message.login')}}</v-btn>
...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$store.dispatch("laravellogin", {
        user
      });
    },
    signInWithFacebook() {
